...
class Verifier:
    ...
    def verify(self, c, s1, s2, s3, s4, T1, T2, T3, m, a, a0, g, h, y, n, lambda_1, lambda_2, gamma_1, gamma_2):
        ...
        c = c % n
        temp1 = ( (utils.square_and_multiply(a0, c, n) * utils.square_and_multiply(T1, s1 - c * two_pow_gamma1, n) % n) * ModularInv((utils.square_and_multiply(a, s2 - c * two_pow_lambda1, n)*utils.square_and_multiply(y, s3, n)) % n, n) ) % n

        temp2 = utils.square_and_multiply(T2, s1 - c * two_pow_gamma1, n) * ModularInv( utils.square_and_multiply(g, s3, n), n ) % n

        temp3 = utils.square_and_multiply(T2, c, n) * utils.square_and_multiply(g, s4, n) % n

        temp4 = utils.square_and_multiply(T3, c, n) * utils.square_and_multiply(g, s1 - c * two_pow_gamma1, n) * utils.square_and_multiply(h, s4, n) % n 
